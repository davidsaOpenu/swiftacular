...
docker version --format 'Docker client {{.Client.Version}} / server {{.Server.Version}}' \
  2>/dev/null || docker version 2>/dev/null | head -4 || true
docker info --format \
...
# Pre-load busybox so that:
#   (a) storage/proxy init containers (busybox:1.36) start without a Docker Hub pull
#   (b) local-path-provisioner helper pods (busybox:latest) can provision PVCs —
#       without this, mariadb/storage/grafana PVCs never bind and pods stay Pending
# CI machines with a corporate HTTPS proxy frequently fail Docker Hub TLS pulls
# from inside kind nodes even when the host Docker daemon works fine.
step "Pre-loading busybox, alpine, and grafana into kind"
docker pull busybox:1.36         &>/dev/null || true
docker pull busybox:latest       &>/dev/null || true
docker pull alpine:3.19          &>/dev/null || true
docker pull grafana/grafana:10.4.3 &>/dev/null || true
kind load docker-image busybox:1.36         --name "${_KIND_CLUSTER}" &>/dev/null || true
kind load docker-image busybox:latest       --name "${_KIND_CLUSTER}" &>/dev/null || true
kind load docker-image alpine:3.19          --name "${_KIND_CLUSTER}" &>/dev/null || true
kind load docker-image grafana/grafana:10.4.3 --name "${_KIND_CLUSTER}" &>/dev/null || true
info "busybox, alpine, and grafana loaded into kind"

# Pre-load remaining local-registry images so imagePullPolicy: IfNotPresent
# finds them in containerd without ever contacting the registry.
# (The registry can return 503 on manifest HEAD checks even when the image data
#  is present — IfNotPresent + pre-load is the only reliable workaround.)
step "Pre-loading swiftacular-proxy into kind"
if docker image inspect "localhost:5001/swiftacular-proxy:latest" &>/dev/null 2>&1; then
  kind load docker-image "localhost:5001/swiftacular-proxy:latest" \
    --name "${_KIND_CLUSTER}" &>/dev/null || true
  info "swiftacular-proxy loaded into kind"
else
  warn "localhost:5001/swiftacular-proxy:latest not found — proxy will pull via registry"
fi

step "Pre-loading swiftacular-package-cache into kind"
if docker image inspect "localhost:5001/swiftacular-package-cache:latest" &>/dev/null 2>&1; then
  kind load docker-image "localhost:5001/swiftacular-package-cache:latest" \
    --name "${_KIND_CLUSTER}" &>/dev/null || true
  info "swiftacular-package-cache loaded into kind"
else
  warn "localhost:5001/swiftacular-package-cache:latest not found — package-cache will pull via registry"
fi
...
_helm_monitor() {
  ...
  while true; do
    ...
    for _pod in $(kubectl get pods -n "${ns}" --no-headers 2>/dev/null \
      ...
      # Try the current init/main container; fall back to previous terminated container.
