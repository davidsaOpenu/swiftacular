...
#
# jb (jsonnet-bundler) is only needed when vendor/ is absent from the repo.
# vendor/ is committed, so normal runs never need jb.
set -euo pipefail
...
if [[ "${NEED_INSTALL}" -eq 1 ]]; then
  if ! command -v jb >/dev/null 2>&1; then
    error "vendor/ is missing and jb (jsonnet-bundler) is not installed."
    error "Install: go install github.com/jsonnet-bundler/jsonnet-bundler/cmd/jb@latest"
    exit 1
  fi
...
if command -v jsonnet >/dev/null 2>&1; then
  ...
  COMPILED=$(docker run --rm \
    ...
    bash -c "
      ...
      apt-get install -y -qq --no-install-recommends jsonnet >/dev/null 2>&1
      n=0
