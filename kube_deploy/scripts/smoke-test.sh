...
# ── proxy bypass ──────────────────────────────────────────────────────────────
# All curl calls in this script target localhost (registry, Keystone NodePort,
# Swift NodePort).  Corporate HTTP proxies on CI machines intercept even
# localhost traffic and return 403/Squid-error pages.
unset_proxy_vars
...
# section: print the header and record a timing checkpoint; the final summary
# prints per-section durations via the lib's print_phase_summary.
section() { echo ""; phase "${*}"; step "${*}"; }
...
if [[ -z "${KS_TOKEN:-}" ]]; then
  ...
  # Generate objects on the host.  dd failure (usually disk full) must abort
  # the stress test with a clear message instead of cascading into confusing
  # "upload errors" downstream.
  ...
  for i in $(seq 1 "${STRESS_OBJECTS}"); do
    if ! dd if=/dev/urandom of="${STRESS_DIR}/obj-${i}.bin" \
         bs=1024 count="${STRESS_SIZE_KB}" 2>/dev/null; then
      _stress_err="object generation failed at obj-${i} (disk full?)"
      echo "  ✗ ${_stress_err} — disk usage:" >&2
      df -h "${STRESS_DIR}" 2>/dev/null | sed 's/^/    /' >&2 || true
      break
    fi
    sha256sum "${STRESS_DIR}/obj-${i}.bin" >> "${STRESS_DIR}/sums.txt"
  done

  # Parallel upload via curl (skipped if generation failed)
  UPL_FAIL=0
  if [[ -z "${_stress_err}" ]]; then
  ...
  if [[ -n "${_stress_err}" ]]; then
    : # generation already failed — verdict carries _stress_err
  elif [[ "${UPL_FAIL}" -ne 0 ]]; then
