...
_ks_auth() {
  ...
  payload=$(jq -cn \
    ...
    '{auth:{identity:{methods:["password"],password:{user:{name:$user,domain:{name:"Default"},password:$pass}}},scope:{project:{name:"demo",domain:{name:"Default"}}}}}')
  ...
  proj_id=$(printf '%s\n' "${result}" | awk 'p; /^\r?$/{p=1}' | jq -r '.token.project.id' 2>/dev/null)
...
cleanup() {
  ...
  if [[ -n "${KS_TOKEN:-}" ]] && [[ -n "${PROJECT_ID:-}" ]]; then
    local _url="http://localhost:8080/v1/AUTH_${PROJECT_ID}"
...
KS_CODE=$(curl -so /dev/null -w "%{http_code}" "http://localhost:5000/v3" 2>/dev/null || echo 0)
if [[ "${KS_CODE}" == "200" ]]; then
...
PR_CODE=$(curl -so /dev/null -w "%{http_code}" "http://localhost:8080/healthcheck" 2>/dev/null || echo 0)
if [[ "${PR_CODE}" == "200" ]]; then
...
# Swift operations use the NodePort at localhost:8080 which is fine — those
...
if [[ -n "${KS_TOKEN}" ]] && [[ -n "${PROJECT_ID}" ]]; then
  SWIFT_URL="http://localhost:8080/v1/AUTH_${PROJECT_ID}"
  _e2e_err=""
