...
# Always use BuildKit. The legacy builder (DOCKER_BUILDKIT=0) has a known bug
# where multi-stage builds fail with "unknown parent image ID" when an
# intermediate stage's cached image has been evicted from the daemon store.
# BuildKit manages its own content-addressed cache and never hits this issue.
